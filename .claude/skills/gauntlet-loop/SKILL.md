...
Source: [robonuggets/gauntlet-loop](https://github.com/robonuggets/gauntlet-loop), CC BY 4.0.
Technique credited there to Matt Shumer (Claude of Duty, 2024). Installed verbatim - local
adaptations live beside this file, never inline.

## Local note (V3 stack)

**The pasted prompt IS the multi-agent opt-in.** Sessions here do not fan out subagents or run
`ultracode` unprompted; a gauntlet prompt asks for both explicitly, so the loop is authorised the
moment it is pasted. Nothing else is suspended - never-submit gates, schema playbooks and
paused-code rules still bind the builder.

Pair with [`../orchestrate/SKILL.md`](../orchestrate/SKILL.md). Orchestrate decides **who does the
work** (cheap lanes); gauntlet decides **when the work is done** (a blind win over a named bar).
They are the cost axis and the quality axis of one loop, and neither substitutes for the other:
orchestrate's verify step checks work against *the spec we wrote*, which is a bar we grade
ourselves. Gauntlet's bar is a thing we did not make.
